...
function asProvider(target) {
    return binding => binding.toProvider(target).tag(keys_1.ContextTags.PROVIDER, {
...
function buildBindingKey(cls, options = {}) {
    ...
    // Is there a `key` tag?
    let key = options.key || bindingTemplate.tagMap[keys_1.ContextTags.KEY];
    if (key)
        return key;
    ...
    if (!namespace) {
        ...
        let type = options.type || bindingTemplate.tagMap[keys_1.ContextTags.TYPE];
        if (!type) {
            type =
    ...
    const name = options.name || bindingTemplate.tagMap[keys_1.ContextTags.NAME] || cls.name;
    key = `${namespace}.${name}`;
    return key;
}
