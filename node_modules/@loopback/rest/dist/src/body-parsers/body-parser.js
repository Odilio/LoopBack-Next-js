...
let RequestBodyParser = class RequestBodyParser {
    ...
    async _matchRequestBodySpec(operationSpec, request) {
        ...
        if (!operationSpec.requestBody)
            return { requestBody };
        const contentType = body_parser_helpers_1.getContentType(request) || 'application/json';
...
RequestBodyParser = __decorate([
    __param(0, context_1.inject.tag(types_1.REQUEST_BODY_PARSER_TAG, { optional: true })),
...
function sortParsers(parsers) {
    return parsers.sort((a, b) => {
        const i1 = body_parser_helpers_1.builtinParsers.names.indexOf(a.name);
        const i2 = body_parser_helpers_1.builtinParsers.names.indexOf(b.name);
        return i1 - i2;
    });
}
