...
let Bootstrapper = class Bootstrapper {
    ...
    async boot(execOptions, ctx) {
        const bootCtx = ctx || new context_1.Context(this.app);
        ...
        if (execOptions && execOptions.booters) {
            ...
            mixins_1._bindBooter(this.app, booter));
        }
        ...
        const phases = execOptions
            ? execOptions.filter && execOptions.filter.phases
                ...
                : interfaces_1.BOOTER_PHASES
            : interfaces_1.BOOTER_PHASES;
        // Find booters registered to the BOOTERS_TAG by getting the bindings
        const bindings = bootCtx.findByTag(keys_1.BootBindings.BOOTER_TAG);
        // Prefix length. +1 because of `.` => 'booters.'
        const prefix_length = keys_1.BootBindings.BOOTER_PREFIX.length + 1;
        // Names of all registered booters.
        const defaultBooterNames = bindings.map(binding => binding.key.slice(prefix_length));
        ...
        const filteredBindings = bindings.filter(binding => names.includes(binding.key.slice(prefix_length)));
