 ...
 */
class Annotation {
    constructor(comment, doc) {
        this.setProperty = this._setProperty.bind(this);
        this.tagParsers = {
            ...
            promise: (tag, ctx) => {
                let resolveObject = { name: 'resolve' };
                if (!tag.types) {
                    this.parseTagForType(tag);
                    if (tag.name)
                ...
                if (!('description' in resolveObject)) {
                    console.warn('Description for resolve object not found in %s', ctx.string);
                }
            },
        ...
        tags.forEach(tag => {
            ...
            let shouldParse = typeof tag === 'object' && typeof tag.type === 'string' && tag.type;
            if (shouldParse) {
        ...
        let desc = attrs.description /* ngdoc style */ ||
